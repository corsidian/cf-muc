...
<%@ page import="net.emiva.util.ParamUtils,
                 ...
                 net.emiva.crossfire.forms.spi.*,
                 net.emiva.crossfire.forms.*,
                 org.dom4j.Element,
                 org.xmpp.packet.IQ,
    ...
    errorPage="error.jsp"
%>
<%@ page import="net.emiva.crossfire.plugin.muc.NotAllowedException"%>
...
<%  // Get parameters
    ...
    MUCRoom room = null;
    if (!create) {
        room = plugin.getMultiUserChatManager().getMultiUserChatService(roomJID).getChatRoom(roomName);
        ...
        if (create && errors.size() == 0) {
            ...
            if (errors.size() == 0) {
                ...
                if (room != null) {
                    ...
                    try {
                        room = plugin.getMultiUserChatManager().getMultiUserChatService(roomJID).getChatRoom(roomName, address);
...
<meta name="pageID" content="plugin-muc-room-create"/>
<% } else { %>
<title><fmt:message key="muc.room.edit.form.edit.title"/></title>
<meta name="subPageID" content="plugin-muc-room-edit-form"/>
<% } %>
