...
<%@ page import="net.emiva.util.ParamUtils,
                 net.emiva.util.AlreadyExistsException,
                 java.util.*,
...
<%  // Get parameters
    ...
    if (!create && !plugin.getMultiUserChatManager().isServiceRegistered(mucname)) {
    ...
    if (!create && mucdesc == null) {
        mucdesc = plugin.getMultiUserChatManager().getMultiUserChatService(mucname).getDescription();
    }
    ...
    if (save) {
        ...
        if (errors.size() == 0) {
            if (!create) {
                plugin.getMultiUserChatManager().updateMultiUserChatService(mucname, mucname, mucdesc);
                // Log the event
                ...
                try {
                    plugin.getMultiUserChatManager().createMultiUserChatService(mucname, mucdesc, false);
                    // Log the event
...
<% if (create) { %>
<meta name="pageID" content="plugin-muc-service-create"/>
<% } else { %>
<meta name="subPageID" content="plugin-muc-service-edit-form"/>
